...
public class TextFieldValidator {
    ...
    public static final Pattern ovhPassword = Pattern.compile("^[a-zA-Z0-9]{9,30}$");

    public static BooleanBinding emptyTextAreaBinding(TextArea textArea, String message, Map<BooleanBinding, String> messages) {
        BooleanBinding binding = Bindings.createBooleanBinding(()
                -> textArea.getText().trim().isEmpty(), textArea.textProperty());
        configureTextAreaBinding(binding, textArea, message, messages);
        return binding;
    }
    ...
    public static void configureTextAreaBinding(BooleanBinding binding, TextArea textField, String message, Map<BooleanBinding, String> messages) {
        messages.put(binding, message);
        if (textField.getTooltip() == null) {
            textField.setTooltip(new Tooltip());
        }
        String tooltipText = textField.getTooltip().getText();
        binding.addListener((obs, oldValue, newValue) -> {
            updateTextAreaValidationStatus(textField, tooltipText, newValue, message);
        });
        updateTextAreaValidationStatus(textField, tooltipText, binding.get(), message);
    }
    ...
    public static void updateTextAreaValidationStatus(TextArea textField,
            String defaultTooltipText, boolean invalid, String message) {
        textField.pseudoClassStateChanged(PseudoClass.getPseudoClass("validation-error"), invalid);
        String tooltipText;
        if (invalid) {
